        ...
        {data.map((item, index) => (
          <a href={`${item.path}`} key={index}>
            <div className="w-full p-2 bg-white rounded group dark:bg-dark-surface dark:border dark:border-dark-text/10">
              ...
              <p className="pt-2 italic text-light-text dark:text-dark-text">
                "{item.description}"
              </p>
            </div>
          </a>
  ...
  useEffect(() => {
    ...
    const doSearch = async () => {
      ...
      try {
        ...
        const fuse = new Fuse<ISearch>(searchIndex, {
          threshold: 0.4,
          keys: ['title', 'description', 'pubDate'],
          ignoreLocation: true, // permite que el match esté en cualquier parte del string
      ...
      >
        <div className="flex items-center justify-between relative">
          ...
          <button
            ...
            className="bg-buttons-bg text-white px-3 py-1 rounded cursor-pointer"
          >
            Cancel
          </button>
        </div>
