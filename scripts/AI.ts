...
if (!fs.existsSync(aiEnvPath)) {
  const answers = await inquirer.prompt([
    {
      ...
      mask: '*',
      validate: input => (input.length > 0 ? true : 'The API key cannot be empty'),
    },
  ]);
  ...
  console.log(
    'A .env.ai file has been created at the root of your project containing your API key and has been added to your .gitignore to ensure it isn’t committed to version control.'
  );
}
